...
%   measured s21 before truning on the switch.
...
%     S21_normalizer = ones(size(FrequencyOrig));
%     
%     for freqvalue = 1:size(FrequencyOrig,1)
%         
%         if isempty(find(Frequency == FrequencyOrig(freqvalue,1), 1)) == 0
%            
%             Freq_normalizer(freqvalue,1) = FrequencyOrig(freqvalue,1);
%             S21_normalizer(freqvalue,1) = S21(freqvalue,1);
%             
%         else
%         
%             posavant = find(Frequency < FrequencyOrig(freqvalue,1),1);
%             posapres = find(Frequency > FrequencyOrig(freqvalue,1),1);
%             
%             % Take the averge in case it doesn't exist in the frequency
%             % that we need
%             
%             S21_normalizer(freqvalue,1) =...
%                 (S21(posavant,1) + S21(posapres,1))/2;
%             
%             Freq_normalizer(freqvalue,1) = FrequencyOrig(freqvalue,1);
%             
%         end
