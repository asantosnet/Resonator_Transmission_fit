function [ InfoText ] = plotVNA_readWO
%UNTITLED Summary of this function goes here
%   Detailed explanation goes here
%   Uses the version 3 of the compare_FITS21ResonTiN and reads the file that
%   comes from Romain's manip
%   This only works to read the file in WithoutWO
...
%     S21 = DataText{2} + 1i.*DataText{3};
    ...
    if isempty(posFreq) || isempty(posRe)|| isempty(posIm)
        ...
        Npoints = size(DataText{2},1);
        
        Npowerpoints = 1;

        Power = ones(Npoints,1);
        
        DataTot = cell(3,10*Npowerpoints);
        
        Frequency = DataText{posFreq};
    
        S21 = DataText{posRe} + 1i.*DataText{posIm};
        
    end
    
    
        plotVNATiN('Amplitude S21','S21 - Transmission',['S21','(dB)'],...
        ' Frequency(Hz)',Frequency...
        ,10.*log(S21),'-10',fitYes)
...
function plotVNATiN(figname,measurment_type,labely,labelx,...
   ...
   xmax(pointXmax) = ',';
    ...
    if fitYes == 1 && strcmp('Amplitude S21',figname)
        ifYes(datax,datay,...
...
function ifYes(Datax,Datay,Measurement_Type,Title,Power)
    ...
    compare_FitS21ResonTiN( Datax,Datay,...
...
function [S21normalized,S21_normalizer,Freq_normalizer,norm,failed] = ....
    calc_normalize_S21(DataTot,Npower,treshold)
...
if failed == 0
    ...
    % By linear interpolation, do a fit so that the frequency of the 
    % data used to normalize matches the one to be normalized
